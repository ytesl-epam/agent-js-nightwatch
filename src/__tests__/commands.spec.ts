...
describe('commands', function() {
  jest.spyOn(IPCClient, 'publishIPCEvent').mockImplementation(() => {});
  ...
  describe('rpLog', function() {
    ...
    test('should call log method from ReportingAPI to send log message to reporter', function() {
      ...
      expect(spyReportingApiLog).toHaveBeenCalledWith(
        LOG_LEVELS.INFO,
        'Log message',
        null,
        'parent name',
  ...
  describe('rpScreenshot', function() {
    const screenshotActionData: ScreenshotDataInterface = {
      value: 'file content',
      sessionId: 'unknown',
      status: 'done',
    };
    ...
    test('should call logInfo method from ReportingAPI to send log with screenshot to reporter', function() {
      ...
      expect(spyReportingApiLogInfo).toHaveBeenCalledWith(
        'Screenshot',
        {
          name: 'testScreen',
  ...
  describe('rpSaveScreenshot', function() {
    const spyGetFileMimeType: jest.SpyInstance = jest
      .spyOn(utils, 'getFileMimeType')
