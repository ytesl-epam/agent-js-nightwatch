...
describe('setDefaultFileType', function() {
  ...
  test('should leave received file type if it exists in the file parameter', function() {
    const updatedFile = setDefaultFileType({
      name: 'file',
...
describe('getStartLaunchObj', function() {
  beforeAll(() => {
    jest
      .spyOn(commonUtils, 'getSystemAttributes')
  ...
  test('should return startLaunchObject with system attributes in case of launch has not any attributes', function() {
    ...
    expect(startLaunchObject.attributes).toEqual([{ value: 'mockValue', key: 'mockKey' }]);
  });

  test('should return startLaunchObject with system attributes in case of launch has empty attributes', function() {
...
describe('getCaller', function() {
  test("should return the caller's caller ", function() {
    jest.spyOn(utils, 'getStack').mockReturnValueOnce([
      {
...
describe('getCodeRef', function() {
  ...
  test('invokes buildCodeRef to create code reference based on the test path and item name', function() {
    jest.spyOn(utils, 'getCaller').mockReturnValueOnce({ getFileName: () => 'C:/project/file.js' });
...
describe('calculateTestItemStatus', function() {
  const { calculateTestItemStatus } = utils;
